...
For more information regarding the Accounts Library: https://github.com/corda/accounts/blob/master/docs.md
## Pre-Requisites

For development environment setup: (https://docs.r3.com/en/platform/corda/4.9/community/getting-set-up.html).
...
Go to the Buyer's node terminal and paste in the following code:
```
...
This is creating 3 accounts under Seller's node and sharing with their specific counterparty's node or account.
 ...
 run a vaultQuery to see the records that have been stored at each node by using: 
```
...
```
verify the message had been successfully pass into Buyer's procurement team, you can navigate to Buyer's node terminal and type in: 
```
...
```
run ```flow start ViewInboxByAccount acctname: BuyerFinance``` at Buyer' node terminal
...
```
 run ```flow start ViewInboxByAccount acctname: SellerFinance``` at Seller's node terminal 
...
```
 run ```flow start ViewInboxByAccount acctname: SellerInventory``` at Seller's node terminal 
...
flow start SendShippingRequest whoAmI: SellerInventory, whereTo: BuyerWarehouse, shipper: ShippingCo, Cargo: 10 boxes of Books
 ...
 run ```run vaultQuery contractStateType: ShippingRequestState``` at ShippingCo's node terminal 
...
```
run ```flow start ViewInboxByAccount acctname: BuyerWarehouse``` at Buyer's node terminal 
